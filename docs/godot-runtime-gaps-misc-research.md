...
| **엔진 레벨 리플리케이션 그래프** | 🔴 대규모 | `SceneReplicationInterface` 재작업 + 공간 인덱스 도입 — ~~오픈월드 co-op 규모(4~8인)엔 불필요~~ **[2026-08-25] 멀티플레이 규모가 게임 디자인 미정 항목이라 조건부 유지. 4~8인 확정 시에만 불요, MMO/대규모 확정 시 스코프 내** |
...
- **스트랜드 기반 헤어 + Marschner/Chiang BSDF** — Horizon/UE Groom 급 헤어 파이프라인. ~~Jolt `Physics/Hair` 업스트림 대기~~ → **[2026-08-25 반전] 시뮬은 `Jolt/Physics/Hair` 벤더링(upstream 5.6.0 실재, `thirdparty/README.md:519`), 렌더는 커스텀 스트랜드 래스터라이저 + BSDF.** 대기 항목이 아니라 스코프 내 과제
...
| **8** | **§6 스트랜드 헤어/헤어 BSDF** | AAA 최종 목표(Horizon/UE Groom 급). ~~Jolt 업스트림 대기~~ **[반전] 벤더링 가능** | ~~P3 장기~~ → **스코프 내.** 시뮬=`Jolt/Physics/Hair` 벤더링 / 렌더=스트랜드 래스터+Marschner BSDF. 카드형 헤어는 프로토타입 스톱갭일 뿐 종착점 아님 |
...
| **§5 리플리케이션 그래프** | ~~4~8인 co-op엔 오버엔지니어링~~ → **조건부.** 멀티플레이 규모 미확정이므로 게임 디자인 결정 대기. 4~8인이면 불요, 그 이상이면 스코프 내 |
...
| **§6 캐릭터 — 스트랜드 헤어** | 🔴 "불필요 — 스타일라이즈드 타깃" | 🔴→🟡 **스코프 내 (2026-08-25 재반전)** | Horizon/UE Groom 급 스트랜드 헤어는 AAA 캐릭터의 정점. ~~Jolt `Physics/Hair` 업스트림 대기~~ → **upstream 5.6.0에 실재, Godot이 벤더링 제외**(`thirdparty/README.md:519`) → 번들 확대 + 커스텀 래스터라이저 |
