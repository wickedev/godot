...
> **2026-08-25 작성.** 이 절은 초판에서 플레이스홀더로 비어 있었고, §0 요약표의 *"셋 다 콘솔을 지원하지 않는다"* 는 근거 없이 남아 있었다. 웹 실사로 채운다. **요약표 문구는 부정확했다 — 정확히는 "미들웨어 본체가 아니라 Godot 통합 레이어가 콘솔 미지원"이다.**

### (a) 실사 결과 (2026-08-25)

| 레이어 | Wwise | FMOD | Steam Audio |
|--------|-------|------|-------------|
| **미들웨어 본체의 콘솔 지원** | ✅ PS5/Xbox Series/Switch 공식 (콘솔 개발자 등록 시 SDK 제공) | ✅ 동일 | ⚠️ **공식 콘솔 빌드 없음.** 커뮤니티 문의만 존재, Valve 공식 답 없음. 단 **Apache-2.0 전체 소스 공개** → 자가 포팅 가능 |
| **Godot 통합의 콘솔 지원** | ❌ 커뮤니티 GDExtension(alessandrofama)의 플랫폼 선택지는 **Windows/macOS/Linux/Android/iOS/Web뿐** | ❌ 유사 (커뮤니티 통합) | ❌ 커뮤니티 통합, 데스크톱/모바일 |
| **W4 콘솔 포트와의 관계** | W4는 "middleware-approved console ports"를 표방하나 **오디오 미들웨어 번들을 명시하지 않음** — 계약 시 직접 확인 필요 | 동일 | 해당 없음 (자가 포팅 전제) |

### (b) 재정의된 문제

"미들웨어가 콘솔을 막는다"가 아니라 **"Godot 쪽 통합 레이어를 콘솔 SDK로 다시 빌드하는 작업이 아무에게도 상품화돼 있지 않다"** 이다. 포크 전제에서 이건 라이선스 문제가 아니라 엔지니어링 문제다:
- Wwise/FMOD 경로: 콘솔 개발자 등록(§4 관문) 후 벤더 콘솔 SDK에 GDExtension을 재빌드 — 수 주. 단 클로즈드 소스 의존이 포크·업스트림 디스커넥트 전략과 이질적.
- Steam Audio 경로: 소스가 있으므로 **포크에 벤더링해 콘솔 타깃 포함 자가 빌드** — 이 프로젝트의 다른 모든 서브시스템과 같은 취급. 단 콘솔 인증 통과 사례가 공개돼 있지 않음(리스크).

### (c) §6과의 분업 (변함없음)

어느 미들웨어를 골라도 **§6(스트리밍·보이스 매니저·믹스 스레딩)은 Steam Audio로는 안 닫힌다.** Wwise/FMOD는 §6을 통째로 제공하지만, Steam Audio 선택 시 §6은 코어 개조로 직접 짓는다(2~4개월, (c)표 참조 — 이미 로드맵 L8 Wave 2에 편성됨).

상세 결정 매트릭스: [godot-audio-middleware-console-decision.md](./godot-audio-middleware-console-decision.md)
