...
- **풀 버퍼 1개의 크기는 `LIMIT_MAX_STORAGE_BUFFER_SIZE` 이하여야 한다 (v1.1 확정 — L1 확인 항목의 답, C1 실코드+실측).** 이는 생성 제약이 아니라 **바인딩 제약**이다 — `vertex_buffer_create`는 크기를 검증하지 않고(rendering_device.cpp:3865, p_data 일치 확인이 유일), 엔진은 이 상한을 어디서도 확인하지 않으며(`LIMIT_MAX_STORAGE_BUFFER_SIZE`는 BIND_ENUM_CONSTANT 외 미등장), Godot은 SSBO를 항상 전체 버퍼 범위로 바인딩한다(`range = buffer->size`, rendering_device_driver_vulkan.cpp:4934). 따라서 초과 풀은 **바인딩 시점에 진단 없이** 검증레이어 오류/UB로 실패한다. 실측: Vulkan 경로 4 GiB−1(GB10·MoltenVK 공통), Metal 18.7 GiB → 풀 버퍼당 소스 삼각형 **보수적으로 약 1.04억**(4,294,967,295 B ÷ 41.28 = 104,044,749. §7.7.8 세 표본 중 **심이 많은** 것 기준 — 심 없는 합성 픽스처는 22.86/23.44지만 그 값들은 실제 에셋의 상한이 아니다. 압축 `meshopt_encodeMeshlet` 적용 전 값). **A등급 회람 중 (2026-08-25)** — 종전 1.87억(콘 이전)과 회람 초기의 1.83억(심 없는 표본 기준) 모두 폐기 대상. **1.83억을 근거로 이미 내린 설계 판단이 있으면 재검토할 것 — 43% 낙관적이었다.** 초과가 필요하면 풀을 복수 버퍼로 분할한다 — RD에 부분 범위 SSBO 바인딩이 없다(`uniform_set_create`에 offset/range 부재; 추가 시 L1 작업). §7.6 공유 계약 테스트에 "풀 크기 ≤ 상한" 검증을 포함할 것(엔진이 잡아주지 않는 항목).
...
**§2가 인용할 값은 41.28 B/tri.** 클러스터 카운트가 833→837, 그룹이 106→109로 바뀐 것은 콘 때문이 아니라 같은 커밋의 **접힘 거부(fold rejection)** 때문이다 — 접힌 단순화를 거부하면 그 그룹의 클러스터가 루트로 남아 레벨 구성이 달라진다. 콘은 레코드 크기만 바꾸고 알고리즘에 영향이 없다.
...
| 2026-08-25 | **A (회람 중, 개정)** | §7.7.8 손계산 투영 → 직렬화 실측 교체(노멀 콘 포함) + **심 많은 표본 추가**, §2 상한 1.87억 → **1.04억**. 방법 변경이므로 §9.2상 A. 표본 추가분만 따로 보면 B(같은 방법·조이는 방향)지만 같은 수치를 건드리므로 병합해 회람 | ⏳ C1·C2·C3 재회람 |
