...
- [ ] **L5** — ⭐ **Jolt 벤더링 범위 확대 스파이크** — `Jolt/Compute`·`Jolt/Shaders`·`Jolt/Physics/Hair`를 5.6.0에서 추가 벤더링 + SCons 통합 시도. **저비용·고효과**(§15-B). 동시에 `Physics/SoftBody`의 셀프 콜리전 지원 여부 소스 확인
