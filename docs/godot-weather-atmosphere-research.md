...
> ⛔ **[폐기 — 2026-08-25 사용자 최종 결정]** 이 절 전체는 *"원신급 스타일라이즈드"* 전제에서 작성됐고, 그 전제는 **§0 AAA 재채점이 무효화했다.**
> **확정: 물리 정확 다중산란 대기(Bruneton/Hillaire 4-LUT)와 근경 관통 구름은 둘 다 필수다.**
> **이 절을 계획 근거로 쓰지 말 것** — 총 공수를 **3~4주**로 적고 있으나 §0 AAA 기준은 **8~14개월(1인)** 이다. 견적이 10배 어긋난다.
> **정본: §0 한 장 요약(AAA 기준 재채점).** 실행 순서: [구현 로드맵 마스터](./godot-openworld-implementation-roadmap.md) L5 레인.
> 아래는 **이력 보존용**이다. P0~P2의 *개별 기법*(글로벌 유니폼 버스 · TOD 싱글턴 · 그림자 예산 튜닝 · 레인 오클루전 getter · 탑다운 RT 재사용)은 여전히 유효하며 로드맵 Wave 1~2에 그대로 편성돼 있다. **무효화된 것은 우선순위와 §P4 "만들지 말 것" 판정이다.**
...
| 항목 | 안 만들어도 되는 근거 |
|------|----------------------|
| **물리 정확 다중산란 대기** | ⛔ **[폐기 — 필수로 전환]** AAA는 UE5 Sky Atmosphere(Hillaire 2020 4-LUT) 등가를 요구한다. 사전계산 알고리즘이라 **sky 적용까지 코어 0**, 씬 픽셀 AP만 국소 코어. 6~10주(§0-3) |
| **근경 관통 구름**(`CompositorEffect`, 3~4주) | ⛔ **[폐기 — 필수로 전환]** AAA는 Nubis/UE5급 근경 관통을 요구한다. **`EFFECT_CALLBACK_TYPE_POST_SKY`(`compositor.h:46`) + `access_resolved_depth`로 코어 0**(초판이 이 콜백을 놓쳤다). 반사·GI 반영만 국소 코어. 2~4개월(§0-4) |
| **froxel 확장으로 구름층 커버** | froxel은 프러스텀 정렬 + `volumetric_fog_length`(기본 64m) 제약이라 **구름층 1,500~8,000m는 구조적으로 안 들어온다**(§5 b). 설계 오용 |
| **거리필드 소프트 섀도우 / HW-RT 그림자** | ⛔ **[폐기 — 재개봉]** gaps §9가 AAA 기준으로 재개봉. SDFGI `sdf_tex`(`gi.h:581`) 재사용 = 중간 코어, HW-RT는 씬측 BLAS/TLAS 생명주기만 결손. [lumen 로드맵](./godot-lumen-gi-implementation-roadmap.md) Phase 2가 정본 |
...
> ⛔ **[폐기]** 아래 두 줄은 원신급 하한 전제. **AAA 정본은 §0** — P0~P3 합계 **8~14개월(1인)**, P4의 대기·구름·DF섀도우 3건은 **필수로 전환**됐다.
...
> ⛔ **[반영 완료 + 부분 폐기 — 2026-08-25]** 이 절의 제안은 **gaps 문서 §9(환경 시뮬)로 이미 반영됐고, 그 과정에서 AAA 기준으로 뒤집혔다.**
> 아래 인용 블록의 **🔴 P4 "만들지 말 것"** 과 **"현실적 대안"** 두 항목은 **무효다** — 물리기반 대기와 볼류메트릭 구름(근경 관통 포함)은 AAA에서 **필수**다.
> **정본: [gaps §9](./godot-openworld-engine-gaps.md) + 이 문서 §0.** 아래는 이력 보존용.
...
> - ~~🔴 P4 (**만들지 말 것**): 물리 정확 다중산란 대기, 근경 관통 구름, froxel 확장, 거리필드/HW-RT 그림자, `BaseMaterial3D` 개조, TOD 라이트맵 크로스페이드.~~ ⛔ **[폐기]** 대기·근경구름·DF/RT그림자 3건은 AAA 필수로 전환. froxel 확장·`BaseMaterial3D` 개조·TOD 라이트맵만 여전히 유효.
>
> **현실적 대안 (여전히 유효)**
> - ~~**원신급 목표엔 P0+P1로 충분하고, 물리기반 대기·볼류메트릭 구름은 애초에 만들지 않는 것이 이득이다.**~~ ⛔ **[폐기]** 원신은 하한 참조점일 뿐 목표가 아니다. 둘 다 만든다. `ProceduralSkyMaterial` 그라디언트 + TOD 램프 + 스크롤 구름 판이 스타일라이즈드의 정답이며, 원신도 원거리 그림자를 사실상 포기하고 실내/실외를 레벨 볼륨으로 판정한다.
